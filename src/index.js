...
// promise-ified exec
const exec = (cwd, opts) => new Promise(
	(resolve, reject) => {
		callbackExec(
			cwd,
			Object(opts),
...
const checkNpmCiExist = () => exec('npm ci --help').then(
	output => output.toString() === 'npm ci'
);

const doesPackageLockJsonExist = () => new Promise(
	resolve => {
		access(
			'package-lock.json',
			fsconstants.R_OK,
...
const haveObjectValuesChanged = (dep1, dep2) => Object.keys(
	Object(dep1)
).concat(
	Object.keys(Object(dep2))
).some(
	key => dep1[key] !== dep2[key]
);
